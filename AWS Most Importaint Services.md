...
|                                               | **Cognito**                                     | User authentication and authorisation via user pools; token-based access control for APIs. User registers -> login to Cognito -> cognito returns a token -> call API endpoint with security token           |
